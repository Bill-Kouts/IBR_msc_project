...
import CrawlerService.queue.ByteableImageEntry;
import CrawlerService.queue.ImageQueue;
import IBRService.IBR;
import IBRService.IBREntry;
import IBRService.IBRIndex;
import IBRService.IBRStringRepresentation;
import IBRService.ImageDescriptor;
import PHashService.utils.DCTData;
import PHashService.utils.Utilities;
import PHashService.utils.YCbCrData;
import ij.IJ;
import ij.ImagePlus;
...
public class PHashService {
	...
	public static Map.Entry<IBR, ImageDescriptor> getIBRFromFile(String urlLocation) {
		...
		// If there was a problem opening the file, last error message wont be
		// null
	...
	private static void printOriginalImageData(BufferedImage im) {
		if (PRINT_ORIGINAL_IMAGE_DATA) {
			System.out.println("image name: " + im.toString() + " <<width:" + im.getWidth() + ">> <<height:"
					+ im.getHeight() + ">>");
		}
	}
